...
create table trained_in(
  ...
  primary key (physicianid, treatmentid, certificationdate),
foreign key(physicianid) references physician(employeeid),
...
create table prescribes(
  ...
  appointmentid int,
  dose int,
primary key (physicianid, patientid, medicationid, appointmentid, `date`),
foreign key(physicianid)references physician(employeeid),
foreign key(patientid)references patient(ssn),
...
primary key (nurseid, blockfloorid, blockcodeid, oncallstart),
...
primary key (patientid, procedureid, stayid, physicianid,assistingnurseid, `date`),
foreign key (patientid)references patient(ssn),
